...
-(void)parserDidEndDocument:(NSXMLParser *)parser{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSString* s = [[self.arrData lastObject] valueForKey:@"Price"];
        self.lblLast.text = s;
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(nullable NSString *)namespaceURI qualifiedName:(nullable NSString *)qName attributes:(NSDictionary<NSString *, NSString *> *)attributeDict{
    // If the current element name is equal to "StockandIndexGraphic" then initialize the temporary dictionary.
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(nullable NSString *)namespaceURI qualifiedName:(nullable NSString *)qName{
    if ([elementName isEqualToString:@"StockandIndexGraphic"]) {
        // If the closing element equals to "StockandIndexGraphic" then the all the data of a has been parsed and the dictionary should be added to the data array.
        [self.arrData addObject:[[NSDictionary alloc] initWithDictionary:self.dictTempDataStorage]];
    }
    else if ([elementName isEqualToString:@"Price"]){
        // If the Price element was found then store it.
