...
#import "IMKB50ViewController.h"
#import "ListCell.h"
#import "DetailViewController.h"

@interface IMKB50ViewController ()
@property (weak, nonatomic) IBOutlet UITableView *tbl50;
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"IMKB-50";
...
-(void) get {
    ...
    for (NSDictionary *item in self.arrData) {
        ...
        [self.arrIMKBData addObjectsFromArray: results];//addObjectsfromarray
...
{
    [self performSegueWithIdentifier:@"Detail50" sender:tableView];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    NSIndexPath *myIndexPath = [self.tbl50 indexPathForSelectedRow];
    
    if ([segue.identifier isEqualToString:@"Detail50"]) {
